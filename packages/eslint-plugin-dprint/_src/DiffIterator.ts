import { Diff } from "@tsplus/eslint-plugin-dprint/Diff"
...
export class DiffIterator implements IterableIterator<Diff> {
  ...
  private handleAdd(current: Change): Diff {
    ...
    if (next1 && next2 && !next1.added && !next1.removed && RegularExpression.isWhitespace(next1.value)) {
      // "added → as-is → added" and all of the three are whitespaces.
      // It frequently appears as adding a line break with indentation.
      if (next2.added && RegularExpression.isWhitespace(current.value) && RegularExpression.isWhitespace(next2.value)) {
