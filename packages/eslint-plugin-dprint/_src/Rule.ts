...
import { ConfigSchema } from "@tsplus/eslint-plugin-dprint/ConfigSchema"
import type { Addition, Removal, Replacement } from "@tsplus/eslint-plugin-dprint/Diff"
import { DiffIterator } from "@tsplus/eslint-plugin-dprint/DiffIterator"
import { RegularExpression } from "@tsplus/eslint-plugin-dprint/RegularExpression"
...
function createMessage(diff: Addition | Removal | Replacement): Message {
  switch (diff._tag) {
    ...
    case "Replacement": {
      if (RegularExpression.isWhitespace(diff.oldText) && RegularExpression.isWhitespace(diff.newText)) {
